...
export default function YoungAdult() {
  ...
  const limit = 2; // книг на странице

  useEffect(() => {
    fetch(`http://localhost:5000/api/youngadult?limit=${limit}&page=${page}`)
      .then(res => res.json())
      .then(data => {
        setBooks(data.youngadult|| []);
